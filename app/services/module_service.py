...
def create_module(db: Session, payload: ModuleCreate):
  ...
  stmt = select(Course).where(Course.id.in_(payload.course_ids))
  courses = list(db.scalars(stmt))

  # optional: ensure all IDs existed
  if len(courses) != len(set(payload.course_ids)):
    missing = set(payload.course_ids) - {ct.id for ct in courses}
    raise HTTPException(
  ...
  # fetch the CourseTemplate rows
  stmt = select(ModuleTemplate).where(ModuleTemplate.id == payload.template_id)
