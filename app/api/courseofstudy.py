...
from app.schemas.coursesofstudy import CourseofStudyCreate, CourseofStudyRead
...
def get_course_of_study(courseofstudy_id: int, db: Session = Depends(get_db)):
  courseofstudy = course_of_study_service.get_course_of_study(
    db, courseofstudy_id
  )
...
def create_course_of_study(
  template: CourseofStudyCreate, db: Session = Depends(get_db)
):
  return course_of_study_service.create_course_of_study(db, template)


@router.delete("/{cos_id}")
def delete_courseofstudy(cos_id: int, db: Session = Depends(get_db)):
  return course_of_study_service.delete_course_of_study(db, cos_id)
